...
class SolarWeek {
  ...
  SolarWeek next(int weeks, bool separateMonth) {
    ...
    if (separateMonth) {
      int n = weeks;
      SolarWeek week = SolarWeek.fromYmd(
          solar.getYear(), solar.getMonth(), solar.getDay(), _start);
      int month = _month;
      bool plus = n > 0;
      while (0 != n) {
        solar = solar.next(plus ? 7 : -7);
        week = SolarWeek.fromYmd(
            solar.getYear(), solar.getMonth(), solar.getDay(), _start);
        ...
        if (month != weekMonth) {
          ...
          if (plus) {
            ...
            if (SolarUtil.getWeeksOfMonth(
              ...
              solar = Solar.fromYmd(week.getYear(), week.getMonth(),
  ...
  String toFullString() => 'Năm $_year Tháng $_month Tuần thứ ${getIndex()}';
}
